...
The `GRanges` of the feature `ranges` is only mandatory the 
...
information from some some UCSC genomes can automatically be fetched using
...
`r Rpackage("consensusSeekeR")` package which generate both
...
### Only chrompsome 10 is going to be analysed
chrList <- Seqinfo("chr10", 135534747, NA)
...
```{r parallelExample, collapse=TRUE}
### Load data
...
### Results from both chromosomes are present
results$consensusRanges[9:10]
```
