...
surrounded by genomic ranges. The `consensusSeekeR` package is characterized 
by its use of the position values, instead of the genomic ranges, to identify
...
is used to associate the feature position to its range. Each row of both 
`GRanges` must also be named to using tag that allow the differenciation of 
...
for two or more ChIP-Seq samples from multiple experiments. The peak positions
are the feature used to identify the consensus regions.
