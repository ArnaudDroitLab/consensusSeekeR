...
author: Astrid Louise Deschenes, Fabien Claude Lamaze, Pascal Belleau and Arnaud Droit 
...
Ranges play a fundamental role in the analysis of diffent experimental genomic 
data. As 
an example, the peak calling step, in the analysis of ChIP-seq data, commonly
generates narrow peaks outputs. The NarrowPeak format, wich is used by the 
...
data, is based on counting the alignments overlapping exons.  Those exons can 
easily be translated as genomic ranges.
...
One current approach used to identify consensus in a group of results consist 
of extracting the overlapping regions of the genomic ranges. This approach, 
when use on a large number of experiments, can miss, as a side effect, regions 
when one of the experiment has missing or slightly shift features. On the 
...
identify consensus 
ranges in a group of experiments which generated feature positions
surrounded by a genomic ranges. The `r Rpackage("consensusSeekeR")` package 
is characterized 
by the use of the features positions, instead of genomic ranges, to 
identify the consensus regions.
...
ranges, to identify the consensus regions.

Using iterative steps on ordered features position values from all experiments, 
a window of fixed size (specified by user) with the current feature position 
as starting point is set. All 
features which reside inside the window are gathered to calculate a 
median feature position which is then used to recreate a new window. This time, 
the 
...
moment that 
the set of features become stable. The final set of features positions is
used to fix
...
feature inside it to be retained as an final consensus region. The minimum 
...
region resizing, the iterative steps are not reprocessed. So, it is possible 
that the extension step adds new features in the extended consensus region. 
However, those new features ranges won't be taked into account during the
extension step.
...
* a `GRanges` of the feature positions of all experiments. All entry must have
an unique metadata field called `name`. Each row of the `GRanges` must be named
to identify by the experiment source.
* a `GRanges` of the feature ranges for all experiments. All entry must have
an unique metadata field called `name`. Each row of the `GRanges` must be named
to identify by the experiment source.

Each feature must have one entry in both `GRanges`. The metadata field `name` 
is used to associate the feature position to its range. 

Each row of both 
`GRanges` must also be named using tag that allow the differenciation of 
the experiment sources. This time, all entries from the same experiment must
assigned the same name.

This is an example of a dataset which metadata field `name` has been removed. 
As shown bellow, a metadata field `name` can easily be created. Row names can 
be also easily be assigned.

```{r input, collapse=TRUE}
## Load dataset
...
## Create a new metadata field "name" unique to each entry
A549_FOSL2_01_NarrowPeaks_partial$name = paste0("A549_FOSL2_01_entry_", 
                                1:length(A549_FOSL2_01_NarrowPeaks_partial))

## Assing the same row name to each entry
names(A549_FOSL2_01_NarrowPeaks_partial) = rep("A549_FOSL2", 
                                length(A549_FOSL2_01_NarrowPeaks_partial))

## Final result
head(A549_FOSL2_01_NarrowPeaks_partial, n = 4)
```

## Using consensusSeekeR on nucleosome positions   
...
the reference genome. A range of $\pm$ 73 bps is usually superposed to the
...
adding $\pm$ 73 bps to the detected positions. (TODO: PASCAL info sur 
le chromosome)
...
The metadata field `name` is specific to each software and is incremented to be
unique. For the positions and ranges dataset from the same software, the `name`
field is paired to ensure that each position can be associated to a range.

```{r datasetNuc, collapse=TRUE}
## Each entry for the positions dataset has an equivalent "name" entry in the 
## ranges dataset
head(NOrMAL_nucleosome_positions, n = 2)
head(NOrMAL_nucleosome_ranges, n = 2)

## Entries for each software are unique
head(NOrMAL_nucleosome_positions, n = 2)
...
dataset has to be assigned an name. The positions and ranges
datasets from the same source must be assigned identical row names. In this 
...
A nucleosome from at least 2 software must be present in a region to be 
...
"ENCFF002CFQ", "ENCFF002CFR" ans "ENCFF002CFS" are going to be 
...
![alt text](figures/NR3C1_exp.jpg "ChIP-Seq data from multiple experiments")
...
![alt text](figures/extend.jpg "Effect of expandToFitPeakRegion parameter")
...
```{r graphSizeEffect, warning=FALSE}
