...
One current approach used to identify consensus regions in a group of 
...
* a `GRanges` of the feature `positions` of all experiments with a 
metadata field called `name`. 
* a `GRanges` of the feature `ranges` for all experimentsw ith a metadata 
field called `name`. 
...
entries from the same experiment must assigned the same name.
...
be created. Row names can be also easily be assigned.
